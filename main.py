...
from models import Base, Author, Thesis, University, Institute, Language, Keyword, SubjectTopic, Supervisor
from DTO import *
...
@app.get("/universities/{university_id}/institutes", response_model=List[InstituteResponse])
def get_institutes_by_university(university_id: int, db: Session = Depends(get_db)):
    ...
    if not institutes:
        raise HTTPException(status_code=404, detail="No institutes found for the given university.")
    return institutes
